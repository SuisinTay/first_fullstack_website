...
const BillingPage = async () => {
  ...
  const createSubsription = async () => {
    ...
    const subscriptionUrl = await getStripeSession({
      customerId: dbUser?.stripeCustomer as string,
  ...
  const createCustomerPortal = async () => {
    ...
    const session = await stripe.billingPortal.sessions.create({
      customer: data?.user.stripeCustomer as string,
      ...
      <Card className="flex flex-col">
        <CardContent className="py-8">
          ...
          <p className="mt-5 text-lg text-muted-foreground ">
            Write as many note as you want
          </p>
        </CardContent>
        <div className="flex-1 flex flex-col justify-between px-6 pt-6 pb-8 bg-secondary rounded-lg m-1 space-y-6 sm:p-10 sm:pt-6">
          ...
          <form className="w-full" action={createSubsription}>
            <StripeSubcriptionButton />
          </form>
        </div>
      </Card>
